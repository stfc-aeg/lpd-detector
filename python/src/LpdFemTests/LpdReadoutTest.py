...
def LpdReadoutTest(tenGig, femHost, femPort, destIp, destMac, srcIp, legacyPowerCard):
    ...
    rc = theDevice.open(femHost, femPort, legacyPowerCard=legacyPowerCard)
...
#                print "{0:<32} = {1:<10}".format(param, value)
        ...
        if rc != LpdDevice.ERROR_OK:
            print("ERROR: Failed to read back numberTrains parameter, rc = {}".format(rc))
        ...
        if num_trains_configured == 0:
            print("**** numberTrains = 0, waiting a while before stopping")
            time.sleep(5.0)
...
if __name__ == '__main__':
    ...
    legacy = False  # Legacy power card use
    ...
    if args.legacy:         legacy  = args.legacy
...
#     print ""
    ...
    if ((destIp == None) and destMac) or (destIp and (destMac == None)):
        ...
        if (destIp) and (srcIp== None):
            ...
            LpdReadoutTest(tenGig, femHost, femPort, destIp, destMac, srcIp, legacy)
