"""
    LpdPowerTesting - Developing script(s) to test supermodel's  ASIC (and more?) modules
...
def powerCardTest(femHost, femPort):
    ...
    for sensor in range(numSensors):
        ...
        for paramType in TVCparamsTypes:
            ...
            if rc != LpdDevice.ERROR_OK:    
                print "1.Unable to read parameter %s rc=%d: %s" % (paramName, rc, theDevice.errorStringGet())
                errorCount += 1
                if errorCount > 2:
    ...
    for powerCard in range(numPowerCards):
        ...
        for paramType in miscParamTypes:
            ...
            initResults[paramName] = val
            if rc != LpdDevice.ERROR_OK:    
                print "2.Unable to read parameter %s rc=%d: %s" % (paramName, rc, theDevice.errorStringGet())
                errorCount += 1
                if errorCount > 2:
    ...
    #TODO: Switch voltages on
    # ...
##    os.system("python imageplot.py "+str(args.fileName)+" -t "+str(args.train)+" -i "+str(idx)+" &")

    asicmodule = 0
    hvbias = 50
    thisFem = LpdPowerControl(femHost, femPort, asicmodule)
...
#        print "    Undertemp    Trip = ", initResults['powerCardUnderTemp0'], "\n"    # Redundant - Always "tripped"
    ...
    if bDebug:
        print "------------------------ Init Results: ------------------------"
        displayResult(initResults)
        print "------------------------ Post Results: ------------------------"
        displayResult(postResults)
