...
class LpdAsicControl(object):
    ...
    def __init__(self, xmlObject, pixelFeedbackOverride=-1, pixelSelfTestOverride=-1, debugLevel=0, preambleBit=None, fromFile=False, strict=True):
        ...
        self.defaultParamsList = ['mux_decoder_default', 'feedback_select_default', 'self_test_decoder_default', 'daq_bias_default']
    ...
    def parseElement(self, theElement):
        ...
        if self.debugLevel > 0: print "############################################ parseElement() ############################################"
        ...
        for child in theElement:
            ...
            if cmd in self.paramsDict:
                ...
                # Check if key contain illegal attribute(s) (e.g. _default key containing pixel attribute)
                if not bPixel:
                ...
                (attribValue, attribName) = (-1, "")
                # If pixel tag, obtain pixel value
                if bPixel:
                    attribValue = self.getAttrib(child, 'pixel')
                    (thisIndex, attribName) = (self.doLookupTableCheck(cmd, attribValue), "pixel")
                else:
                    if bIndex:
                        attribValue = self.getAttrib(child, 'index')
                        (thisIndex, attribName) = (self.doLookupTableCheck(cmd, attribValue), "index")
                    else:
                        # No pixel nor index tags: therefore the key's value is an integer
                        (thisIndex, attribName, attribValue) = (0, "", "")
                        
                # Update entry of the dictionary key
                self.setParamValue(cmd, thisIndex, value)

                if self.debugLevel > 0: 
                    print "XML cmd: %28s %6s   %15s" % (cmd, attribName, attribValue), "value = ", value
        ...
        for key in self.defaultParamsList:

            if self.debugLevel > 0: print "1st Pass:  _default key: %28s" % key, " = ",
            ...
            if self.debugLevel > 0: print defaultKeyValue,

            # Is default value not specified ?
            if defaultKeyValue[LpdAsicControl.VALUES] == -1:

                self.setParamValue(key, 0, 0)
                
                if self.debugLevel > 0: print " now = ", self.getParamValue(key)[LpdAsicControl.VALUES]
            else:
                if self.debugLevel > 0: print " (no change)"
        ...
        for key in self.paramsDict:
            ...
            if key.endswith("_default"):
                ...
                if self.debugLevel > 0: print "2nd Pass: Found _default key: %28s" % key, " = ",
                ...
                if isinstance(keyValue[LpdAsicControl.VALUES], types.IntType):
                    ...
                    if self.debugLevel > 0: print "2nd Pass: Found integer type: %21s" % key
...
class LpdAsicControlTest(unittest.TestCase):
    ...
    def testStringParse(self):
        ...
        paramsObj = LpdAsicControl(stringCmdXml, pixelFeedbackOverride=-1, pixelSelfTestOverride=-1, preambleBit=6)
        paramsObj.encode()
...
#        print "\n", digitalControlVals, "\n", expectedVals
    ...
    #                                <mux_decoder_default value="7"/>
        ...
        def testNewCfgIncludingPreambleDelay(self):
            ...
            thisFile = '/LpdFemClient/tests/preambleDelaySlowControl.xml'
            pythonPath = os.environ['PYTHONPATH']
            fullPath = pythonPath + thisFile
        
            theParams = LpdAsicControl(fullPath, preambleBit=6, fromFile=True)
        ...
        def testExternalOverrideValues(self):
            ...
            paramsObj = LpdAsicControl(stringCmdXml, pixelFeedbackOverride=1, pixelSelfTestOverride=4, preambleBit=6)
