...
# Testing new helper class implementation:
...
if __name__ == "__main__":
    ...
    print "~+~+~+~+~ Connecting to host: %s, port: %i, asicmodule: " % (host, port),
    if asicModule == 0:
        print "supermodule",
        numberPowerCards = 2
    elif asicModule == 1:
...
#    femI2cBus == 0: "LM82"
#    femI2cBus == 256: "EEPROM"
#    femI2cBus == 512: "RHS Power Card"
#    femI2cBus == 768: "LHS Power Card"
        
    print "    ~+~+~+~+~+~+~ Connecting.. ~+~+~+~+~+~+~"


    theDevice = LpdDevice()
    rc = theDevice.open(host, 6969, asicModuleType=asicModule)
    ...
    if args.lv:
        ...
        for powerCard in range(numberPowerCards):
            paramName = 'asicPowerEnable'+str(powerCard)
            rc = theDevice.paramSet(paramName, LpdPowerControl.ON)
            if rc!= theDevice.ERROR_OK:
                print "Unable to set parameter '%s' Error number %d: %s." % (paramName, rc, theDevice.errorStringGet())
        ...
        if args.hv:
            ...
            for powerCard in range(numberPowerCards):
                paramName = 'sensorBiasEnable'+str(powerCard)
                rc = theDevice.paramSet(paramName, LpdPowerControl.OFF)
                if rc!= theDevice.ERROR_OK:
                    print "Unable to set parameter '%s' Error number %d: %s." % (paramName, rc, theDevice.errorStringGet())
            ...
            if args.hvbias > -1:
                ...
                for powerCard in range(numberPowerCards):
                    ...
                    rc = theDevice.paramSet(paramName, adcCounts)
                    if rc!= theDevice.ERROR_OK:
                        print "Unable to set parameter '%s' Error number %d: %s." % (paramName, rc, theDevice.errorStringGet())
