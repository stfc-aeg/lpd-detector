...
#                print (5-i),
    ...
    def convertListIntoXmlFile(self, scList):
        ...
        if bMuxControlAllZero:
            ...
            stringList.append( '''   <mux_decoder_pixel_master value="0"/>''' )
        else:
            # ..else populate all 512 keys
            for idx in range(512):
                stringList.append( '''   <%s''' % ExtractSlowControlParamsFromFile.muxDecoderLookupTable[idx] +''' value="%i"/>''' % scList[0+idx] )
        ...
        for idx in range(512*2):
            ...
            stringList.append( '''   <%s''' % ExtractSlowControlParamsFromFile.pixelSelfTestLookupTable[idx] +''' value="%i"/>''' % scList[512+idx] )
        

        # Bias Control configuration
        for idx in range(47):
            stringList.append( '''   <%s''' % ExtractSlowControlParamsFromFile.biasCtrlLookupTable[idx] +''' value="%i"/>''' % scList[1537+idx] )
