	...
	else
	{
		...
		for (unsigned int iRow = 0; iRow < kNumRowsPerAsic; iRow++)
		{
			for (unsigned int iCol = 0; iCol < kNumColsPerAsic; iCol++)
			{
				...
				unsigned short testBit    =  mMpx3PixelConfigCache[chipIdx][pixelTestModeConfig][pixelCacheIdx] & 1;
				unsigned short maskBit    =  mMpx3PixelConfigCache[chipIdx][pixelMaskConfig][pixelCacheIdx] & 1;
				unsigned short configDiscL0 = (mMpx3PixelConfigCache[chipIdx][pixelDiscLConfig][pixelCacheIdx] >> 0) & 1;
				unsigned short configDiscL1 = (mMpx3PixelConfigCache[chipIdx][pixelDiscLConfig][pixelCacheIdx] >> 1) & 1;
				unsigned short configDiscL2 = (mMpx3PixelConfigCache[chipIdx][pixelDiscLConfig][pixelCacheIdx] >> 2) & 1;
				unsigned short configDiscL3 = (mMpx3PixelConfigCache[chipIdx][pixelDiscLConfig][pixelCacheIdx] >> 3) & 1;
				unsigned short configDiscL4 = (mMpx3PixelConfigCache[chipIdx][pixelDiscLConfig][pixelCacheIdx] >> 4) & 1;
				unsigned short configDiscH0 = (mMpx3PixelConfigCache[chipIdx][pixelDiscHConfig][pixelCacheIdx] >> 0) & 1;
				unsigned short configDiscH1 = (mMpx3PixelConfigCache[chipIdx][pixelDiscHConfig][pixelCacheIdx] >> 1) & 1;
				unsigned short configDiscH2 = (mMpx3PixelConfigCache[chipIdx][pixelDiscHConfig][pixelCacheIdx] >> 2) & 1;
				unsigned short configDiscH3 = (mMpx3PixelConfigCache[chipIdx][pixelDiscHConfig][pixelCacheIdx] >> 3) & 1;
				unsigned short configDiscH4 = (mMpx3PixelConfigCache[chipIdx][pixelDiscHConfig][pixelCacheIdx] >> 4) & 1;

				// Build pixel configuration counter values from bit fields
				pixelConfigCounter0[iRow][iCol] = 0;
				pixelConfigCounter1[iRow][iCol] =
						(testBit      << 11) |
						(configDiscH4 << 10) | (configDiscH3 << 9) | (configDiscH2 << 8) | (configDiscH1 << 7) | (configDiscH0 << 6) |
						(configDiscL4 <<  5) | (configDiscL3 << 4) | (configDiscL2 << 3) | (configDiscL1 << 2) | (configDiscL0 << 1) |
						(maskBit << 0);
...
//		std::cout << std::endl;
		...
		this->acquireConfig(ACQ_MODE_UPLOAD, kPixelConfigBufferSizeBytes/2, 2, (kPixelConfigBaseAddr + kPixelConfigBufferSizeBytes), 1);
#endif
