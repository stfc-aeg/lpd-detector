...
ExcaliburFrameDecoder::ExcaliburFrameDecoder () :
    FrameDecoderUDP (), current_frame_seen_ (-1), current_frame_buffer_id_ (-1),
        current_frame_buffer_ (0), current_frame_header_ (0),
...
void ExcaliburFrameDecoder::init (LoggerPtr& logger, bool enable_packet_logging,
    unsigned int frame_timeout_ms)
{
  ...
  if (enable_packet_logging_)
  {
    ...
    LOG4CXX_INFO (packet_logger_,
                  "PktHdr: |               |     DestinationPort");
    LOG4CXX_INFO (
        packet_logger_,
        "PktHdr: |               |     |      SubframeCounter  [4 Bytes]");
    LOG4CXX_INFO (
        packet_logger_,
...
void ExcaliburFrameDecoder::process_packet_header (size_t bytes_received,
    int port, struct sockaddr_in* from_addr)
{
  ...
  if (enable_packet_logging_)
  {
    std::stringstream ss;
    uint8_t* hdr_ptr =
        reinterpret_cast<uint8_t*> (current_packet_header_.get ());
    ss << "PktHdr: " << std::setw (15) << std::left
        << inet_ntoa (from_addr->sin_addr) << std::right << " " << std::setw (5)
        << ntohs (from_addr->sin_port) << " " << std::setw (5) << port
        << std::hex;
    for (unsigned int hdr_byte = 0; hdr_byte < sizeof(Excalibur::PacketHeader);
        hdr_byte++)
    {
      if (hdr_byte % 8 == 0)
      {
  ...
  LOG4CXX_DEBUG_LEVEL (
      3,
      logger_,
      "Got packet header:" << " packet: " << packet_number << " subframe ctr: "
          << subframe_counter << " idx:" << subframe_idx << " SOF: "
          << (int) start_of_frame_marker << " EOF: "
          << (int) end_of_frame_marker);
    ...
    if (frame_buffer_map_.count (current_frame_seen_) == 0)
    {
      if (empty_buffer_queue_.empty ())
      {
        ...
        if (!dropping_frame_data_)
        {
          LOG4CXX_ERROR (
              logger_,
              "First packet from frame " << current_frame_seen_
      ...
      else
      {
        ...
        current_frame_buffer_ = buffer_manager_->get_buffer_address (
        ...
        if (!dropping_frame_data_)
        {
          LOG4CXX_DEBUG_LEVEL (
              ...
              logger_,
              "First packet from frame " << current_frame_seen_
                  << " detected, allocating frame buffer ID "
                  << current_frame_buffer_id_);
        }
        else
        {
          ...
          LOG4CXX_DEBUG_LEVEL (
              2,
              logger_,
              "Free buffer now available for frame " << current_frame_seen_
                  << ", allocating frame buffer ID "
                  << current_frame_buffer_id_);
        }
      }
      ...
      current_frame_header_ =
          reinterpret_cast<Excalibur::FrameHeader*> (current_frame_buffer_);
      current_frame_header_->frame_number = current_frame_seen_;
      current_frame_header_->frame_state =
          FrameDecoder::FrameReceiveStateIncomplete;
      current_frame_header_->packets_received = 0;
      current_frame_header_->sof_marker_count = 0;
      current_frame_header_->eof_marker_count = 0;
      memset (current_frame_header_->packet_state, 0,
              Excalibur::num_frame_packets);
      gettime (
          reinterpret_cast<struct timespec*> (&(current_frame_header_->frame_start_time)));
    ...
    size_t bytes_received)
{
  ...
  FrameDecoder::FrameReceiveState frame_state =
      FrameDecoder::FrameReceiveStateIncomplete;
  ...
  if (get_end_of_frame_marker ())
  {
    ...
    size_t payload_bytes_received = bytes_received
        - sizeof(Excalibur::PacketHeader);
    ...
    uint32_t frame_number = static_cast<uint32_t> ((trailer->frame_number
        & 0xFFFFFFFF) - 1);
    LOG4CXX_DEBUG_LEVEL (
        3, logger_, "Subframe EOF trailer has frame number = " << frame_number);
    ...
    else
    {
      if (frame_number != current_frame_header_->frame_number)
      {
        LOG4CXX_WARN (
            logger_,
            "Subframe EOF trailer frame number mismatch for frame "
  ...
  if (current_frame_header_->packets_received == Excalibur::num_frame_packets)
  {
        ...
        || (current_frame_header_->eof_marker_count != Excalibur::num_subframes))
    {
      LOG4CXX_WARN (
          logger_,
          "Incorrect number of SOF (" << current_frame_header_->sof_marker_count
    ...
    if (!dropping_frame_data_)
    {
      ...
      ready_callback_ (current_frame_buffer_id_,
                       current_frame_header_->frame_number); //current_frame_seen_);
...
void ExcaliburFrameDecoder::monitor_buffers (void)
{
  ...
  while (buffer_map_iter != frame_buffer_map_.end ())
  {
    ...
    Excalibur::FrameHeader* frame_header =
        reinterpret_cast<Excalibur::FrameHeader*> (buffer_addr);

    if (elapsed_ms (frame_header->frame_start_time, current_time)
        > frame_timeout_ms_)
    {
      LOG4CXX_DEBUG_LEVEL (
          1,
          logger_,
          "Frame " << frame_num << " in buffer " << buffer_id << " addr 0x"
              << std::hex << buffer_addr << std::dec << " timed out with "
              << frame_header->packets_received << " packets received");
  ...
  if (frames_timedout)
  {
    LOG4CXX_WARN (
        logger_,
        "Released " << frames_timedout << " timed out incomplete frames");
  }
  ...
  LOG4CXX_DEBUG_LEVEL (
      2,
      logger_,
      get_num_mapped_buffers () << " frame buffers in use, "
...
{
  return reinterpret_cast<Excalibur::PacketHeader*> (current_packet_header_.get ())->packet_number_flags
      & Excalibur::packet_number_mask;
}
...
unsigned int ExcaliburFrameDecoder::elapsed_ms (struct timespec& start,
    struct timespec& end)
{
