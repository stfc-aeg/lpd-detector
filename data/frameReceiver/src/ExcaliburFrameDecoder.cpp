    ...
    struct sockaddr_in* from_addr)
{
  ...
  LOG4CXX_DEBUG_LEVEL(3, logger_,
      "Got packet header:" << " packet: " << packet_number << " subframe ctr: " << subframe_counter
          << " idx:" << subframe_idx << " SOF: " << (int) start_of_frame_marker << " EOF: "
    ...
    else
    {
      if (frame_number != current_frame_header_->frame_number)
      {
        LOG4CXX_WARN(logger_, "Subframe EOF trailer frame number mismatch for frame "
            << current_frame_seen_ << ": got " << frame_number << ", expected "
            << current_frame_header_->frame_number);
      }
    }
  }
  ...
  if (current_frame_header_->packets_received == Excalibur::num_frame_packets)
  {
        ...
        || (current_frame_header_->eof_marker_count != Excalibur::num_subframes))
    {
      LOG4CXX_WARN(logger_, "Incorrect number of SOF ("
         << current_frame_header_->sof_marker_count << ") or EOF ("
         << current_frame_header_->eof_marker_count << "markers seen on completed frame "
         << current_frame_seen_);
    }
  ...
  while (buffer_map_iter != frame_buffer_map_.end())
  {
    ...
    if (elapsed_ms(frame_header->frame_start_time, current_time) > frame_timeout_ms_)
    {
      LOG4CXX_DEBUG_LEVEL(1, logger_, "Frame " << frame_num << " in buffer " << buffer_id
          << " addr 0x" << std::hex
          << buffer_addr << std::dec << " timed out with " << frame_header->packets_received
          << " packets received");
