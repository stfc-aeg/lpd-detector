...
class TBTest():
    ...
    def set_10g_structs_variables(self):
        ...
        run_params = {'run_type' : 0,   # 3 for farm mode with nic lists ; 2 for farm mode with fixed ip host and multiple ports ; 1 for legacy non farm mode (only 1 ip host and port per link)  
                      'num_cycles'  : 1,
    ...
    # link 1 is at bottom of board when in crate
        ...
        x10g_5 = {'fpga_mac' : self.mac_addr_to_uint64('62-00-00-00-00-03'),   
                        ...
                        'data_gen' : 1, # data generator    
                        'data_format' : 0, # data format type   
                        'frame_len'  : 0x100,    # frame len in bytes   # 125000*8000
                        'num_frames' : 1,    # number of frames to send
...
#-------------------------------------------------------------------------------------    
  ...
  # TBHost now set in run_params structure
        ...
        response = myTBClient.rdmaWrite(0x0600000f, ppc_nr-1)  # selects either ppc1 or ppc2 term output
...
#        # Resets ppc 
#        myTBClient.toggle_bits(9, 1)        
#        print "Resetting PPC 1.."
    ...
    def config_10g_link(self, run_params, x10g, myTBClient):
        ...
        if x10g['enable'] == 1:
            ...
            if x10g['data_gen'] == 2:   # data source is ppc
                ...
                # Resets ppc 
                myTBClient.toggle_bits(9, ppc_reset_bit)        
                theDelay = 10
