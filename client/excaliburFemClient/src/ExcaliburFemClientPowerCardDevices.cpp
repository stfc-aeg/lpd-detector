...
float ExcaliburFemClient::powerCardMonitorRead(excaliburPowerCardMonitor aMonitor)
{
	...
	if (monitorMap.empty())
	{
		...
		monitorMap[humidityMonitor]       = powerCardAD7998Map( 1, 4, kAD7998RawToHumidity, kAD7998HumidtyOffset );
...
void ExcaliburFemClient::powerCardPCF8475BitWrite(int aBit, int aVal)
{
	...
	std::cout << "Old value: 0x" << std::hex << (int)response[0]
	          << " other write bits: 0x" << (int)otherWriteBits
			  << " new value: 0x" << (int)cmd[0] << std::dec << std::endl;
